"""AMG config loader with JSON Schema validation."""
...
def load_amg_config(
    ...
    root = _repo_root()
    config_path = Path(path) if path is not None else root / "configs" / "amg_config.default.json"
    schema_file = (
        Path(schema_path)
        if schema_path is not None
        else root / "contracts" / "AMG_CONFIG_SM_V1.schema.json"
    )
