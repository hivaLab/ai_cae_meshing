...
def test_ansa_pyramid_solid_is_counted_when_pynastran_does_not_keep_it(tmp_path: Path):
    bdf = """BEGIN BULK
MAT1,1,210000.0,,0.3,7.85e-9
GRID,1,,0.,0.,0.
GRID,2,,1.,0.,0.
...
GRID,5,,0.5,0.5,1.
PSOLID,10,1
PYRAMID,100,10,1,2,3,4,5
ENDDATA
"""
    path = tmp_path / "ansa_pyramid.bdf"
    path.write_text(bdf, encoding="utf-8")
    result = validate_bdf(path)
    assert result.passed
    assert result.solid_element_count == 1
